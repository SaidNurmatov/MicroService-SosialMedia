...
```
    ...
    mongod: 
    db.createUser({user: "said",pwd: "root",roles: ["readWrite","dbAdmin"]}) 
...
````bash
docker build --platform linux/amd64 -t devsaid34/egitimconfigserver:v01 .


docker build --platform linux/amd64 -t devsaid34/egitimauth:v02 .


docker build --platform linux/amd64 -t devsaid34/egitimuser:v02 .
